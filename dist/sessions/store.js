...
export async function getOrCreateSession(platform, channelId, userId, config) {
  ...
  `).get(platform, channelId);
    if (existing) {
        if (shouldResetSession(existing)) {
            ...
            database.run("UPDATE sessions SET last_activity = ? WHERE id = ?", [Date.now(), existing.id]);
...
export async function createBackgroundSession(platform, channelId, userId, parentSessionId) {
    const database = await initSessionStore();
    const id = generateSessionId();
    const now = Date.now();
    const session = {
...
export async function getSession(sessionId) {
    const database = await initSessionStore();
    const row = database.query("SELECT * FROM sessions WHERE id = ?").get(sessionId);
...
export async function getPendingBackgroundResults() {
    const database = await initSessionStore();
